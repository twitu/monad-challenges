...
set1Test = testCase "Set 1 tests" $ do
  ...
  assertEqual "rand Even Odd and Ten with seed 1 test" (let v1 = evalGen randEven' $ mkSeed 1
                                                            v2 = evalGen randOdd' $ mkSeed 1
                                                            v3 = evalGen randTen' $ mkSeed 1
                                                      in v1 * v2 * v3) 189908109902700
  assertEqual "randPair with seed 1 test" (evalGen randPair $ mkSeed 1) ('l', 282475249)
  assertEqual "generalPair and randPair comparison test" (evalGen randPair $ mkSeed 1) (evalGen (generalPair randLetter (Gen rand)) $ mkSeed 1)
  assertEqual "generalPair 2 and generalPair comparison test" (evalGen (generalPair randLetter (Gen rand)) $ mkSeed 1) (evalGen (generalPair2 randLetter (Gen rand)) $ mkSeed 1)
  assertEqual "repRandom and 3 randLetter comparison test" (evalGen (repRandom (replicate 3 randLetter)) $ mkSeed 1) randString3
